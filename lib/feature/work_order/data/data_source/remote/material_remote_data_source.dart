...
import 'package:project_mobile_pdam/feature/work_order/data/models/material_model.dart';
import 'package:project_mobile_pdam/feature/work_order/data/models/peminjaman_material_model.dart';
...
class MaterialRemoteDataSource extends RemoteDatasource {
  ...
  Future<DataState<List<MaterialModel>>> getMasterMaterials() async {
    try {
      ...
      return DataFailed(DioException(error: e, requestOptions: RequestOptions(path: '/v1/master/material')));
    }
  }

  Future<DataState<List<PeminjamanMaterialModel>>> getPeminjamanByWo(int workOrderId) async {
    try {
      final response = await get(path: '/v1/workorder/$workOrderId/peminjaman-material');
      final data = response.data['data'] as List;
      final result = data.map((e) => PeminjamanMaterialModel.fromMap(e)).toList();
      return DataSuccess(result);
    } catch (e) {
      return DataFailed(DioException(error: e, requestOptions: RequestOptions(path: '/v1/workorder/$workOrderId/peminjaman-material')));
    }
  }
  ...
  Future<DataState<PeminjamanMaterialModel>> kembalikanMaterial({
    ...
    try {
      ...
      return DataFailed(DioException(error: e, requestOptions: RequestOptions(path: '/v1/peminjaman-material/$peminjamanId/kembalikan')));
    }
  }
}
