...
class MaterialBloc extends Bloc<MaterialEvent, MaterialState> {
  ...
  Future<void> _onGetMasterMaterials(GetMasterMaterialsEvent event, Emitter<MaterialState> emit) async {
    emit(MaterialLoading());
    final result = await getMasterMaterials(const NoParams());
    if (result is DataSuccess && result.data != null) {
      emit(MasterMaterialsLoaded(result.data!));
    } else {
      emit(MaterialError(result.error?.message ?? 'Gagal mengambil data material'));
    }
  }

  Future<void> _onGetPeminjamanByWo(GetPeminjamanByWoEvent event, Emitter<MaterialState> emit) async {
    emit(MaterialLoading());
    final result = await getPeminjamanByWo(event.workOrderId);
    if (result is DataSuccess && result.data != null) {
      emit(PeminjamanLoaded(result.data!));
    } else {
      emit(MaterialError(result.error?.message ?? 'Gagal mengambil histori peminjaman'));
    }
  }

  Future<void> _onPinjamMaterial(PinjamMaterialEvent event, Emitter<MaterialState> emit) async {
    emit(MaterialLoading());
  ...
  Future<void> _onKembalikanMaterial(KembalikanMaterialEvent event, Emitter<MaterialState> emit) async {
    emit(MaterialLoading());
    ...
    if (result is DataSuccess && result.data != null) {
      emit(MaterialActionSuccess('Berhasil mengembalikan material', result.data!));
    } else {
      emit(MaterialError(result.error?.message ?? 'Gagal mengembalikan material'));
    }
  }
}
